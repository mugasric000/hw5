...
if ( !empty($_POST)) { // if $_POST filled then process the form
	...
	if ($valid) { // if valid user input update the database
		...
		$sql = "UPDATE fr_order  set order_date = ?, order_time = ?, order_address = ?, order_description = ? WHERE id = ?";
		$q = $pdo->prepare($sql);
		$q->execute(array($date,$time,$location,$description,$id));
		Database::disconnect();
		header("Location: fr_order.php");
	}
} else { // if $_POST NOT filled then pre-populate the form
	$pdo = Database::connect();
	$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
	$sql = "SELECT * FROM fr_order where id = ?";
	$q = $pdo->prepare($sql);
	$q->execute(array($id));
		...
		<div class="span10 offset1">
			...
			<form class="form-horizontal" action="fr_event_update.php?id=<?php echo $id?>" method="post">
				...
				<div class="control-group <?php echo !empty($dateError)?'error':'';?>">
					...
					<div class="controls">
						<input name="order_date" type="date"  placeholder="Date" value="<?php echo !empty($date)?$date:'';?>">
				...
				<div class="control-group <?php echo !empty($timeError)?'error':'';?>">
					...
					<div class="controls">
						<input name="order_time" type="time" placeholder="Time" value="<?php echo !empty($time)?$time:'';?>">
				...
				<div class="control-group <?php echo !empty($locationError)?'error':'';?>">
					...
					<div class="controls">
						<input name="order_address" type="text" placeholder="Location" value="<?php echo !empty($location)?$location:'';?>">
				...
				<div class="control-group <?php echo !empty($descriptionError)?'error':'';?>">
					...
					<div class="controls">
						<input name="order_description" type="text" placeholder="Description" value="<?php echo !empty($description)?$description:'';?>">
				...
				<div class="form-actions">
					...
					<a class="btn" href="fr_order.php">Back</a>
				</div>
