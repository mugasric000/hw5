...
$sql = "SELECT * FROM delivery_person where id = ?";
$q = $pdo->prepare($sql);
$q->execute(array($id));
$data = $q->fetch(PDO::FETCH_ASSOC);
//--------------new

$sql = "SELECT images.* FROM images, fr_person where fr_person.id= images.new_id AND fr_person.id=?";
$q = $pdo->prepare($sql);
	...
	<body>
		<div class="container">
			...
			<div class="form-horizontal" >
				...
				<?php
					$pdo = Database::connect();
					$sql = "SELECT * FROM fr_assignments, fr_order WHERE assign_event_id = fr_order.id AND assign_per_id = " . $id . " ORDER BY order_date ASC, order_time ASC";
					$countrows = 0;
					foreach ($pdo->query($sql) as $row) {
						echo Functions::dayMonthDate($row['order_date']) . ': ' . Functions::timeAmPm($row['order_time']) . ' - ' . $row['order_address'] . ' - ' . $row['order_description'] . '<br />';
						$countrows++;
					}
