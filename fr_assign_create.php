...
<body>
    <div class="container">
		...
		<div class="span10 offset1">
			<div class="row">
				<h3>Assign a Driver to an Order</h3>
			</div>
					...
					<div class="controls">
						<?php
							...
							if($eventid) // if $_GET exists restrict event options to selected event (from $_GET)
								foreach ($pdo->query($sql) as $row) {
									if($eventid==$row['id'])
									echo "<option value='" . $row['id'] . " '> " . Functions::dayMonthDate($row['order_date']) . " (" . Functions::timeAmPm($row['order_time']) . ") - " .
									trim($row['order_description']) . " (" . 
									trim($row['order_location']) . ") " .
									"</option>";
								}
							else
								foreach ($pdo->query($sql) as $row) {
