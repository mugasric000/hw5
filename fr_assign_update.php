			...
			<form class="form-horizontal" action="fr_assign_update.php?id=<?php echo $id?>" method="post">
				...
				<div class="control-group">
					...
					<div class="controls">
						<?php
							$pdo = Database::connect();
							$sql = 'SELECT * FROM delivery_person ORDER BY lname ASC, fname ASC';
				...
				<div class="control-group">
					...
					<div class="controls">
						<?php
							$pdo = Database::connect();
							$sql = 'SELECT * FROM fr_order ORDER BY order_date ASC, order_time ASC';
							echo "<select class='form-control' name='event_id' id='event_id'>";
							foreach ($pdo->query($sql) as $row) {
								if($row['id']==$event) {
									echo "<option selected value='" . $row['id'] . " '> " . Functions::dayMonthDate($row['order_date']) . " (" . Functions::timeAmPm($row['order_time']) . ") - " . trim($row['order_description']) . " (" . trim($row['order_address']) . ") " . "</option>";
								}
								else {
									echo "<option value='" . $row['id'] . " '> " . Functions::dayMonthDate($row['order_date']) . " (" . Functions::timeAmPm($row['order_time']) . ") - " . trim($row['order_description']) . " (" . trim($row['order_address']) . ") " . "</option>";
								}
							}
							echo "</select>";
							Database::disconnect();
